...
const WireAuthenticationScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const authenticateWire = async (code: string) => {
    ...
    try {
      ...
      try {
        ...
        if (parsedData.type === 'kimson_reward' && parsedData.rewardId) {
        ...
        if (code.startsWith('KIMSON_') && code.length >= 10) {
          await processWireAuthentication(code, 50, 'Kimson Copper Wire', null);
          return;
        }
  ...
  const processWireAuthentication = async (code: string, points: number, wireInfo: string, wireData: any) => {
    ...
    try {
      ...
      Alert.alert(
        errorTitle,
        `${errorMessage}\n\nPlease check:\n• Code is entered correctly\n• Product is genuine Kimson wire\n• Code format: KIMSON_WIRE_[BATCH]_[YEAR]`,
        [
          {
            text: 'Try Again',
            onPress: () => {
              ...
              <View style={styles.inputGroup}>
                ...
                <View style={[
                  ...
                  <TextInput
                    ...
                    placeholder="Enter the code from wire packaging (e.g., KIMSON_WIRE_BATCH123_2024)"
