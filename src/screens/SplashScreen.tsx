...
const SplashScreen: React.FC<Props> = ({ navigation }) => {
  ...
  // Animated values for static KIMSON text bounce effect
  const staticKimsonAnims = useRef(
    ['K', 'I', 'M', 'S', 'O', 'N'].map(() => new Animated.Value(0))
  ).current;
  
  // Create animated values for each letter in KIMSON
  ...
  useEffect(() => {
    ...
    // Animate static KIMSON text (up-down bounce effect)
    const staticKimsonAnimations = staticKimsonAnims.map((anim, index) => {
    ...
    Animated.parallel(staticKimsonAnimations).start();

    // Stop the bounce animations after 2 seconds
    setTimeout(() => {
      staticKimsonAnims.forEach((anim) => {
        Animated.timing(anim, {
          toValue: 0,
          duration: 300,
          useNativeDriver: true,
  ...
  }, [navigation, fadeAnim, scaleAnim, letterAnims, taglineFadeAnim, textOpacityAnim, logoOpacityAnim, staticKimsonAnims, user, rollingSequences]);
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      >
        {/* Rolling KIMSON Text - Letter by Letter */}
