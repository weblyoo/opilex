...
const RegistrationDetailsScreen: React.FC<Props> = ({ navigation, route }) => {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      console.log('💾 [ADMIN] Saving registration data:', {
        userType: userType,
      ...
      console.log('💾 [ADMIN-REGISTRATION] Saving to Firestore with userType:', normalizedUserType);
      await setDoc(
        doc(db, 'users', user.id),
        firestoreUpdateData,
        { merge: true }
      );
      console.log('✅ [ADMIN-REGISTRATION] Firestore update completed');

      // Update local user profile - CRITICAL: Ensure userType is included
      console.log('✅ [ADMIN] Registration data saved to Firestore, updating local profile...');
      await updateUserProfile({
        ...registrationData,
        userType: normalizedUserType, // Explicitly include userType
        createdAt: user.createdAt,
      });
      console.log('✅ [ADMIN] User profile updated with userType:', normalizedUserType);
      
      // Force refresh user data from Firestore to ensure consistency
      console.log('🔄 [ADMIN-REGISTRATION] Refreshing user data from Firestore...');
      const { getDoc } = await import('firebase/firestore');
      const userDocRef = doc(db, 'users', user.id);
      const userDocSnapshot = await getDoc(userDocRef);
      if (userDocSnapshot.exists()) {
        const latestUserData = userDocSnapshot.data();
        console.log('✅ [ADMIN-REGISTRATION] Verified userType in Firestore:', latestUserData.userType);
      }
      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <SafeAreaView style={styles.safeArea}>
          ...
          <View style={styles.cardContainer}>
            <View style={[styles.registrationCard, { backgroundColor: '#1a1a1a' }]}>
              <ScrollView 
                ref={scrollViewRef}
              ...
              <View style={styles.formSection}>
                ...
                <TouchableOpacity
                  ...
                  activeOpacity={0.8}
                >
                  <Text style={[styles.continueButtonText, globalTextStyles.button, { color: '#1a1a1a' }]}>
