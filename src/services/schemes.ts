...
export const schemeService = {
    ...
    try {
      ...
      if (error?.code === 'failed-precondition' || error?.message?.includes('index')) {
        try {
          ...
          console.error('Error loading schemes:', fallbackError);
          return [];
        }
      }
  ...
  async createScheme(scheme: Omit<Scheme, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
    try {
      const schemeData = {
        ...scheme,
        startDate: Timestamp.fromDate(scheme.startDate as Date),
        endDate: Timestamp.fromDate(scheme.endDate as Date),
  ...
  async uploadSchemeImage(file: File, schemeId?: string): Promise<string> {
    // Use a generic 'schemes' kind for storage
