...
export async function uploadImage(file: File, kind: 'pricelist'|'products'|'schemes'): Promise<string> {
  let path: string;
  if (kind === 'schemes') {
    path = `schemes/images/${Date.now()}_${file.name}`;
  } else {
    path = `documents/${kind}/images/${Date.now()}_${file.name}`;
  }
  const r = ref(storage, path);
