...
async function createAdmin() {
  try {
    ...
    if (error.code === 'auth/operation-not-allowed') {
      ...
      console.log('      https://console.firebase.google.com/project/opilex-2a79f/authentication/providers');
      ...
      console.log('   Attempting to sign in and add to admins collection...\n');
      
      try {
        // Try to sign in with the provided password
        const signInCredential = await signInWithEmailAndPassword(auth, email, password);
        const existingUserId = signInCredential.user.uid;
        
        console.log('✅ Signed in successfully');
        console.log('📝 Adding user to admins collection...\n');
        
        // Add to admins collection
        await setDoc(doc(db, 'admins', existingUserId), {
          email,
          role: 'superAdmin',
          name: signInCredential.user.displayName || 'Admin User',
        ...
        console.log('\n⚠️  Could not automatically add user to admins.');
        console.log('   Use addExistingAdmin.js script instead:');
        console.log(`   node scripts/addExistingAdmin.js ${email} ${password}`);
