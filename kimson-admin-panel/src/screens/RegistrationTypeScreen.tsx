...
const RegistrationTypeScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const handleElectricianPress = async () => {
    ...
    console.log('💾 [ADMIN-REGISTRATION-TYPE] Register as Electrician');
    
    if (user?.id) {
      try {
        const normalizedUserType = 'electrician';
        console.log('💾 [ADMIN-REGISTRATION-TYPE] Saving userType to Firestore:', {
        ...
        console.log('✅ [ADMIN-REGISTRATION-TYPE] UserType saved successfully:', normalizedUserType);
      } catch (error) {
        console.error('❌ [ADMIN-REGISTRATION-TYPE] Error saving userType:', error);
      }
    }
  ...
  const handleDealerPress = async () => {
    ...
    console.log('💾 [ADMIN-REGISTRATION-TYPE] Register as Dealer');
