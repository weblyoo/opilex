  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="User Details" size="lg">
      <div className="space-y-6">
        {/* User Info */}
        <div className="bg-gradient-to-br from-black/70 to-black/50 border border-white/10 rounded-2xl p-6 backdrop-blur-sm">
          ...
          <div className="space-y-5">
            <div>
              ...
              <input
                ...
                className="w-full px-5 py-4 border border-white/10 rounded-xl bg-[#111111]/70 backdrop-blur-sm text-white placeholder-white/40 focus:border-[#E31E24] focus:outline-none focus:ring-2 focus:ring-[#E31E24]/20 font-ubuntu-light transition-all duration-300"
              />
            </div>
            <div>
              <label className="block text-sm font-ubuntu-bold mb-3 text-white uppercase tracking-wide">
          ...
          <div className="space-y-3 max-h-64 overflow-y-auto pr-2">
            {rewards.length === 0 ? (
              ...
              rewards.slice(0, 10).map((reward) => (
                <div
                  key={reward.id}
                  className="flex items-center justify-between p-4 bg-[#1A1A1A]/50 rounded-xl border border-white/10 hover:border-white/10 transition-all group"
                >
                  <div>
          ...
          <div className="space-y-3 max-h-64 overflow-y-auto pr-2">
            {authentications.length === 0 ? (
              ...
              authentications.slice(0, 10).map((auth) => (
                <div
                  key={auth.id}
                  className="p-4 bg-[#1A1A1A]/50 rounded-xl border border-white/10 hover:border-white/10 transition-all group"
                >
                  <p className="text-white font-ubuntu-medium text-sm group-hover:text-white transition-colors">
