  ...
  return (
    <Layout>
      <div className="p-8 min-h-screen bg-gradient-to-br from-[#111111] via-[#111111] to-[#0F0F0F] animate-fadeIn">
        ...
        <div className="bg-gradient-to-br from-[#1A1A1A] to-[#111111] border border-white/10 rounded-2xl p-6 backdrop-blur-sm glow mb-6">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
            <input type="date" value={from} onChange={e=>setFrom(e.target.value)} className="px-4 py-3 rounded-xl bg-black/50 border border-white/10 text-white" />
            <input type="date" value={to} onChange={e=>setTo(e.target.value)} className="px-4 py-3 rounded-xl bg-black/50 border border-white/10 text-white" />
            <select value={type} onChange={e=>setType(e.target.value as any)} className="px-4 py-3 rounded-xl bg-black/50 border border-white/10 text-white">
              <option value="all">All</option>
              <option value="reward">Rewards</option>
              <option value="withdrawal">Withdrawals</option>
            </select>
            <input placeholder="Search by userId" value={search} onChange={e=>setSearch(e.target.value)} className="px-4 py-3 rounded-xl bg-black/50 border border-white/10 text-white" />
            <button onClick={load} className="px-5 py-3 bg-[#E31E24] text-white rounded-xl font-ubuntu-bold">Refresh</button>
          </div>
        </div>
