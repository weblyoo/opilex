...
export const PriceList = () => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const data = await getDocument('pricelist');
      setMeta(data);
    } catch (error) {
  ...
  const handleImageUpload = async () => {
    ...
    try {
      const imageUrl = await uploadImage(imageFile, 'pricelist');
      const currentTitle = meta?.title || imageFile.name;
      const currentPdfUrl = meta?.url || '';
      await setDocument('pricelist', currentTitle, currentPdfUrl, imageUrl);
      await load();
      setImageFile(null);
      // Reset file input
      const fileInput = document.getElementById('price-list-image') as HTMLInputElement;
  ...
  const handlePdfUpload = async () => {
    ...
    try {
      const pdfUrl = await uploadPdf(pdfFile, 'pricelist');
      const currentImageUrl = meta?.imageUrl || '';
      await setDocument('pricelist', pdfFile.name, pdfUrl, currentImageUrl);
      await load();
      setPdfFile(null);
      // Reset file input
      const fileInput = document.getElementById('price-list-pdf') as HTMLInputElement;
      if (fileInput) fileInput.value = '';
    } catch (error: any) {
  ...
  const handleBothUpload = async () => {
    ...
    try {
      ...
      const [imageUrl, pdfUrl] = await Promise.all([
        uploadImage(imageFile, 'pricelist'),
        uploadPdf(pdfFile, 'pricelist')
      ]);
      
      // Save both URLs together
      await setDocument('pricelist', pdfFile.name, pdfUrl, imageUrl);
      await load();
      
      // Reset file inputs
      setImageFile(null);
      setPdfFile(null);
      const imageInput = document.getElementById('price-list-image') as HTMLInputElement;
      const pdfInput = document.getElementById('price-list-pdf') as HTMLInputElement;
  ...
  return (
    <div className="p-8 min-h-screen bg-gradient-to-br from-[#111111] via-[#111111] to-[#0F0F0F] animate-fadeIn">
      <div className="mb-10">
        ...
        <p className="text-white/60 font-ubuntu-light">Upload price list image and PDF file (both will be displayed together in the mobile app)</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-gradient-to-br from-[#1A1A1A] to-[#111111] border border-white/10 rounded-2xl p-6 backdrop-blur-sm glow">
          <h2 className="text-2xl font-ubuntu font-black mb-4 text-white">Price List Image</h2>
        ...
        {/* PDF Upload Section */}
        <div className="bg-gradient-to-br from-[#1A1A1A] to-[#111111] border border-white/10 rounded-2xl p-6 backdrop-blur-sm glow">
          <h2 className="text-2xl font-ubuntu font-black mb-4 text-white">Price List PDF</h2>
          <div className="space-y-4">
            {meta?.url && (
              <div className="mb-4">
        ...
        <div className="pt-4 border-t border-white/10">
          ...
          {meta?.imageUrl && meta?.url && (
            <p className="text-green-400 text-sm mt-2 font-ubuntu-light">
              📱 Firestore Path: settings/documents/pricelist/latest
            </p>
          )}
        </div>
      </div>
