...
import { getProductCatalogs, type AdminDocMeta } from '../services/documents';

type CatalogItem = AdminDocMeta & { id: string };
...
const ProductCatalogScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setList([]);
      setLoadError('Unable to load catalogs. Pull to retry.');
    }
  }, []);
  ...
  const openPDFModal = (item: CatalogItem) => {
  ...
  const handleOpenPDF = async () => {
    ...
    try {
      ...
      if (supported) {
        ...
        Alert.alert(
          'Cannot Open PDF',
          'Please install a PDF viewer app to view the catalog.',
          [{ text: 'OK' }]
        );
      }
    } catch (error) {
      ...
      >
        {loading ? (
          <View style={styles.centered}>
            ...
            <Text style={[styles.loadingText, { color: theme.colors.accent }]}>Loading catalogs...</Text>
          </View>
        ) : loadError ? (
          ...
          <View style={[styles.emptyCard, { borderColor: isDark ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.1)' }]}>
            <Icon name="product-catalogue" size={48} color={theme.colors.accent} strokeWidth={2} />
            <Text style={[styles.emptyTitle, { color: theme.colors.text }]}>No catalogs yet</Text>
            <Text style={[styles.emptySub, { color: theme.colors.accent }]}>
              When admin adds product catalogs, they will appear here.
            </Text>
          </View>
        ) : (
                ...
                {item.url ? (
                  ...
                  >
                    ...
                    <View style={styles.pdfTextWrap}>
                      <Text style={[styles.pdfLabel, { color: theme.colors.text }]}>Product catalog (PDF)</Text>
        ...
        <View style={styles.modalOverlay}>
          ...
          >
            <View style={[styles.pdfModalIconOuter, { backgroundColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.06)' }]}>
              <Icon name="product-catalogue" size={48} color={theme.colors.text} strokeWidth={2.5} />
            </View>
            <Text style={[styles.pdfModalTitle, { color: theme.colors.text }]}>Product Catalog PDF</Text>
            <Text style={[styles.pdfModalMessage, { color: theme.colors.accent }]}>
              Open this catalog in your browser or PDF app?
            </Text>
