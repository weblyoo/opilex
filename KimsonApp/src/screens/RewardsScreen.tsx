...
const RewardsScreen: React.FC<Props> = ({ navigation }) => {
  ...
  // Refresh when screen comes into focus (e.g., after QR scan) - with cooldown to prevent loops
  useFocusEffect(
    React.useCallback(() => {
      if (user) {
        const now = Date.now();
        ...
        if (timeSinceLastRefresh > REFRESH_COOLDOWN) {
          console.log('🔄 [REWARDS] Screen focused, refreshing rewards...');
          lastRefreshTime.current = now;
          loadRewards();
          loadScanHistory();
        } else {
          console.log('⏭️ [REWARDS] Skipping refresh - too soon since last refresh');
        }
      }
  ...
  const loadRewards = async () => {
    ...
    try {
      console.log('📊 [REWARDS] Loading rewards for user:', user.id);
      console.log('📊 [REWARDS] Current user points:', user.rewardPoints || 0);
      
      // Try to load from Firestore first
      try {
        const { rewardService } = await import('../services/firestore');
        const recentRewards = await rewardService.getUserRewards(user.id);
        console.log('✅ [REWARDS] Loaded rewards from Firestore:', recentRewards.length);
        
        setRewards(recentRewards);
        // Use user.rewardPoints from Firestore (most accurate)
        ...
        const todayRewards = recentRewards.filter(r => {
          const rewardDate = r.createdAt instanceof Date 
            ? r.createdAt 
            : r.createdAt?.toDate 
              ? r.createdAt.toDate() 
        ...
        const monthRewards = recentRewards.filter(r => {
          const rewardDate = r.createdAt instanceof Date 
            ...
            : r.createdAt?.toDate 
              ? r.createdAt.toDate() 
              : new Date(r.createdAt);
          return rewardDate >= thisMonth;
        });
        
        setDailyPoints(todayRewards.reduce((sum, r) => sum + (r.points || 0), 0));
        setMonthlyPoints(monthRewards.reduce((sum, r) => sum + (r.points || 0), 0));
        
      } catch (firestoreError: any) {
        console.warn('⚠️ [REWARDS] Firestore error, using mock:', firestoreError.message);
