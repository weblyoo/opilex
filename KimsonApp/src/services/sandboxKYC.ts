...
export async function verifyAadhaarOTP(
  ...
  try {
    ...
    // The API might expect reference_id as a number or string depending on the response format
    // Try multiple formats to ensure compatibility
    let referenceId: number | string = requestId;
    
    // Try to parse as integer first (most common format)
    const parsedId = parseInt(requestId, 10);
    if (!isNaN(parsedId) && parsedId.toString() === requestId.trim()) {
      // It's a valid integer string, use the number
      referenceId = parsedId;
      console.log('✅ Parsed reference_id as integer:', referenceId);
    } else {
      // Keep as string if it's not a pure integer
      console.log('ℹ️ Using reference_id as string:', referenceId);
    }
    
    // Try both formats - some APIs accept either
    const requestBody: any = {
      reference_id: referenceId,
      otp: otp.trim(), // Ensure OTP is trimmed
    };
    
    // Some APIs also accept request_id as an alternative
    // We'll use reference_id as primary, but log both for debugging
    console.log('📤 Verification request body (OTP hidden):', {
      ...requestBody,
      otp: '******',
      reference_id_type: typeof referenceId,
    });

    console.log('🔐 Verifying Aadhaar OTP:', {
      requestId: requestId,
      referenceId: referenceId,
      otp: otp.substring(0, 2) + '****', // Only show first 2 digits for security
    ...
    if (!response.ok) {
      ...
      console.error('❌ Sandbox OTP verification error:', {
      ...
      const isOTPError = errorMsgLower.includes('invalid otp') || 
                        ...
                        errorMsgLower.includes('invalid or expired') ||
                        (errorMsgLower.includes('invalid') && errorMsgLower.includes('otp')) ||
      ...
      if (isOTPError) {
        ...
        console.error('❌ OTP Error detected - not falling back to mock:', errorMessage);
        throw new Error(errorMessage || 'Invalid or expired OTP. Please request a new OTP.');
      }
    ...
    console.log('📥 Sandbox Verify Response Data:', JSON.stringify(responseData, null, 2));
